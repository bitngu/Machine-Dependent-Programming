#int** matMult(int **a, int num_rows_a, int num_cols_a, int** b, int num_rows_b, int num_cols_b){
 ...
 #   //Initialize the values to 0 in case malloc be playing
  #  for (int i = 0; i <  num_rows_a; ++i) {
...
matMult:
	prolouge:
		push %ebp
		movl %esp, %ebp
		subl  $ws * 4, %esp #Establishing space for local variables, there are 4
		#Can't i just push ebx, edi, esi? instead using movl? Yes. And if i had used move, i would need to add 			space for the locals. Push creattes space AND pushes on the stack 
		push %ebx
		push %edi
		push %esi
        		...
        		jge first_inner_for_end
        		 ...
        		 second_inner_for_start:
        		 	...
        		 	movl (%edi, %ecx, ws), %edi
        		 	 ...
        		 	 #Need to place EAX  AND EDX into memory to multiply because imull will overwrite these 				registers, EDX for the upper 32 and EAX for the lower 32
        		 	 movl %edx, j(%ebp)
        		 	 movl %eax, i(%ebp)
